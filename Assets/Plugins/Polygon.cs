	...
	public void ParsePolygonsFromFile () {
		...
		if(keyNames != null) {
			for(int i = 0; i<keyNames.Count; i++) {
				...
				List<PolygonPath> totalPathinAllFixtures = new List<PolygonPath>();

				foreach(Hashtable fixture1 in fixtures) {
					...
					totalPathinAllFixtures.AddRange(totalPaths);
				}

				_totalPolygonsinFile[i].paths = totalPathinAllFixtures.ToArray();
